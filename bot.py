...
def process_mpesa_number(message):
    ...
    if withdrawal_id:
        markup.row(
            types.InlineKeyboardButton("✅ Mark Paid", callback_data=f"approvewithdraw_{withdrawal_id}"),
            types.InlineKeyboardButton("❌ Reject", callback_data=f"rejectwithdraw_{withdrawal_id}")
        )
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("approvewithdraw_"))
def approve_withdrawal_callback(call):
    ...
    bot.send_message(call.message.chat.id, f"✅ Withdrawal #{withdrawal_id} marked paid.")
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("rejectwithdraw_"))
def reject_withdrawal_callback(call):
    ...
    bot.answer_callback_query(call.id, "Withdrawal rejected — funds refunded.")
    bot.send_message(call.message.chat.id, f"❌ Withdrawal #{withdrawal_id} rejected, {amount:,} Plats refunded to user.")
...
def withdrawals(message):
    ...
    for wid, user, amount, phone in rows:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.row(
            types.InlineKeyboardButton("✅ Mark Paid", callback_data=f"approvewithdraw_{wid}"),
            types.InlineKeyboardButton("❌ Reject", callback_data=f"rejectwithdraw_{wid}")
        )
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("approvedeposit_"))
def approve_deposit_callback(call):

    if call.from_user.id != ADMIN_ID:
        return

    deposit_id = call.data.split("_")[1]
    ...
    bot.send_message(call.message.chat.id, f"✅ Deposit approved — {amount} credited to {user_id}.")
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("rejectdeposit_"))
def reject_deposit_callback(call):
    ...
    bot.send_message(call.message.chat.id, "❌ Deposit rejected.")
