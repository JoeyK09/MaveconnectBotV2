...
# ================= HOURLY MARKET INSIGHTS (VIP + FREE) =================
# Posts automatically to both groups every hour. VIP gets a full breakdown
# (signal, trend, support/resistance); the free group gets a lighter price
# update. Both start with the coins that moved the most in the last 24h.
#
# How to find a chat ID: add the bot to the group as admin, send any message
# in that group, then visit https://api.telegram.org/bot<YOUR_TOKEN>/getUpdates
# in a browser and look for "chat":{"id": -100XXXXXXXXXX, ...} — that
# negative number is the chat ID.
VIP_GROUP_CHAT_ID = -1004399159423   # e.g. -1001234567890
FREE_GROUP_CHAT_ID = -1004410094067  # e.g. -1009876543210

# ================= GROUP JOIN TRACKING =================
# Logs everyone who joins the VIP or Free group, with a timestamp, so you
# have a running history (not just a snapshot) — and pings you immediately
# when someone joins VIP specifically.
try:
    cursor.execute("""
    ...
    print("airdrop_claims table setup error:", repr(e))
...
try:
    ...
    print("group_joins table setup error:", repr(e))



# Whitelist of coins eligible to be picked as "trending" each cycle — every
# symbol here must already exist in COINPAPRIKA_IDS.
INSIGHT_COINS = [
...
def buy_pickaxe(msg):
    ...
    if balance < price:
        bot.reply_to(
            ...
            reply_markup=buy_plats_nudge_markup()
        )
        return
...
def withdraw_menu():
    ...
    markup.row(
        KeyboardButton("📱 M-Pesa Withdraw"),
...
@bot.message_handler(func=lambda m: m.text == "🎁 USDT Airdrop")
def airdrop_claim_start(message):
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("⚫ OKX", callback_data="airdrop_okx"),
        types.InlineKeyboardButton("🟡 Binance", callback_data="airdrop_binance"),
    )
    bot.send_message(
        message.chat.id,
        "🎁 USDT Trading Airdrop\n\n"
        "Trade $50+ on OKX or Binance using our referral link, then claim here.\n\n"
        f"⚫ OKX: {OKX_REFERRAL}\n"
        f"🟡 Binance: {BINANCE_REFERRAL}\n\n"
        "Already traded $50+? Tap your exchange below to submit a claim — "
        "we'll verify manually and send your USDT reward.",
        reply_markup=markup
    )


@bot.callback_query_handler(func=lambda c: c.data in ("airdrop_okx", "airdrop_binance"))
def airdrop_claim_submit(call):
    exchange = "OKX" if call.data == "airdrop_okx" else "Binance"
    user_id = str(call.from_user.id)
    username = call.from_user.username or ""

    try:
        cursor.execute(
            "INSERT INTO airdrop_claims (user_id, username, exchange) VALUES (%s, %s, %s) RETURNING id",
            (user_id, username, exchange)
        )
        claim_id = cursor.fetchone()[0]
        conn.commit()
    except Exception as e:
        print("airdrop_claims insert error:", repr(e))
        bot.answer_callback_query(call.id, "Something went wrong — try again shortly.")
        return

    bot.answer_callback_query(call.id, "Claim submitted!")
    bot.send_message(
        call.message.chat.id,
        f"✅ Claim #{claim_id} submitted for {exchange}.\n\n"
        f"We'll verify your $50+ trade volume and send your USDT reward once approved."
    )

    admin_markup = types.InlineKeyboardMarkup()
    admin_markup.add(
        types.InlineKeyboardButton("✅ Approve", callback_data=f"approveairdrop_{claim_id}"),
        types.InlineKeyboardButton("❌ Reject", callback_data=f"rejectairdrop_{claim_id}")
    )
    handle = f"@{username}" if username else user_id
    try:
        bot.send_message(
            ADMIN_ID,
            f"🎁 New airdrop claim #{claim_id}\n\n"
            f"👤 {handle} ({user_id})\n"
            f"🏦 Exchange: {exchange}\n\n"
            f"Check your {exchange} affiliate dashboard for this user's volume before approving. "
            f"Approving here does NOT send USDT automatically — you'll still need to send it manually.",
            reply_markup=admin_markup
        )
    except Exception as e:
        print("Admin airdrop notify error:", repr(e))


@bot.callback_query_handler(func=lambda c: c.data.startswith("approveairdrop_") or c.data.startswith("rejectairdrop_"))
def airdrop_claim_review(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "Admins only.")
        return

    approve = call.data.startswith("approveairdrop_")
    claim_id = call.data.split("_", 1)[1]
    new_status = "approved" if approve else "rejected"

    try:
        cursor.execute(
            "UPDATE airdrop_claims SET status=%s WHERE id=%s RETURNING user_id, exchange",
            (new_status, claim_id)
        )
        row = cursor.fetchone()
        conn.commit()
    except Exception as e:
        print("airdrop_claims update error:", repr(e))
        bot.answer_callback_query(call.id, "Update failed.")
        return

    if not row:
        bot.answer_callback_query(call.id, "Claim not found.")
        return

    claim_user_id, exchange = row
    bot.answer_callback_query(call.id, f"Marked {new_status}.")
    bot.edit_message_text(
        f"🎁 Claim #{claim_id} ({exchange}) — {new_status.upper()}",
        call.message.chat.id,
    ...
    try:
        if approve:
            bot.send_message(
                int(claim_user_id),
                f"🎉 Your {exchange} airdrop claim was approved! Your USDT reward is on its way."
            )
        else:
            bot.send_message(
                int(claim_user_id),
                f"❌ Your {exchange} airdrop claim couldn't be verified. Message support if you think this is a mistake."
            )
    except Exception as e:
        print("Airdrop claimant notify error:", repr(e))


def weekly_leaderboard_prize():
    """Announces the top miner once a week (Monday, server time) and pings
    the admin to send their USDT prize — sending USDT itself stays a manual
    step since it touches a real wallet."""
    ...
    while True:
        try:
            ...
            if now.weekday() == 0 and now.hour == 9 and week_key != last_announced_week:
                top = leaderboard(limit=1)

                if top:
                    winner_id, winner_balance = top[0]
                    last_announced_week = week_key

                    announcement = (
                        f"🏆 Weekly Mining Champion!\n\n"
                        f"👤 User {winner_id}\n"
                        f"💰 {winner_balance:,} Plats\n\n"
                        f"🎁 Prize: $5 USDT — sent by the team shortly. Congrats! 🎉"
                    )

                    if VIP_GROUP_CHAT_ID:
                        try:
                            bot.send_message(VIP_GROUP_CHAT_ID, announcement)
                        except Exception as e:
                            print("Weekly prize VIP announce error:", repr(e))
                    if FREE_GROUP_CHAT_ID:
                        try:
                            bot.send_message(FREE_GROUP_CHAT_ID, announcement)
                        except Exception as e:
                            print("Weekly prize Free announce error:", repr(e))

                    try:
                        bot.send_message(
                            ADMIN_ID,
                            f"🏆 Weekly mining winner: {winner_id} ({winner_balance:,} Plats).\n"
                            f"Remember to send their $5 USDT prize manually."
                        )
                    except Exception as e:
                        print("Weekly prize admin notify error:", repr(e))

        except Exception as e:
            print("Weekly Leaderboard Prize Error:", repr(e))

        time.sleep(1800)  # check twice an hour — cheap, and catches the Monday 9am window reliably


@bot.message_handler(content_types=['new_chat_members'])
def track_group_joins(message):
    if message.chat.id == VIP_GROUP_CHAT_ID:
        group_name = "VIP"
    elif message.chat.id == FREE_GROUP_CHAT_ID:
        group_name = "Free"
    else:
        group_name = "Other"
    ...
    for member in message.new_chat_members:
        if member.is_bot:
            continue  # skips the bot's own account joining, and any other bots
        ...
        try:
            cursor.execute(
                """
                INSERT INTO group_joins (user_id, username, full_name, group_name, chat_id)
                VALUES (%s, %s, %s, %s, %s)
                """,
                (member.id, member.username or "", full_name, group_name, message.chat.id)
            )
            conn.commit()
        except Exception as e:
            print("group_joins insert error:", repr(e))
        ...
        if group_name == "VIP":
            ...
            try:
                ...
                print("Admin VIP join notify error:", repr(e))


@bot.message_handler(commands=["members"])
def view_recent_joins(message):
    if message.from_user.id != ADMIN_ID:
        return

    try:
        cursor.execute("""
            SELECT username, full_name, group_name, joined_at
            FROM group_joins
        ...
        bot.reply_to(message, f"❌ Could not fetch joins: {e}")
        return

    if not rows:
        bot.reply_to(message, "No group joins logged yet.")
        return

    lines = ["👥 Last 25 group joins:\n"]
    for username, full_name, group_name, joined_at in rows:
        handle = f"@{username}" if username else (full_name or "Unknown")
        lines.append(f"• {handle} — {group_name} — {joined_at.strftime('%Y-%m-%d %H:%M')} UTC")
...
""",
        ...
        bot.send_message(
            ...
            f"📢 Free Group:\n{FREE_GROUP}\n\n"
            f"💎 VIP Group:\n{VIP_GROUP}\n\n"
            f"💵 Want more Plats to play with? Tap \"Buy Plats\" in the menu below anytime.\n\n"
...
def buy_plats_nudge_markup():
    """An inline 'Buy Plats' button to attach to any insufficient-balance message."""
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("💵 Buy Plats Now", callback_data="quick_buy_plats"))
    return markup


@bot.callback_query_handler(func=lambda c: c.data == "quick_buy_plats")
def quick_buy_plats_callback(call):
    bot.answer_callback_query(call.id)
    user = str(call.from_user.id)  # NOTE: uses call.from_user, not call.message.from_user
    buy_plats_state[user] = {}
    msg = bot.send_message(
        call.message.chat.id,
        "💵 How many Plats would you like to buy?\n\n(1 Plat = 1 KSh)",
...
@bot.message_handler(func=lambda m: m.text == "📱 M-Pesa Withdraw")
...
# ---- Main menu shortcuts: same flows as above, one tap closer ----
@bot.message_handler(func=lambda m: m.text == "💵 Buy Plats")
def buy_plats_menu_shortcut(message):
    buy_plats_start(message)


@bot.message_handler(func=lambda m: m.text == "💸 Redeem Plats")
def redeem_plats_menu_shortcut(message):
    mpesa_withdraw(message)
...
MEMECOIN_TEASER = "🪙 Psst — a Plats memecoin is in the works. No promises on value or timing, but the miners stacking the most Plats now won't be forgotten. Keep swinging that pickaxe. ⛏️"
...
def get_trending_coins(limit=3):
    """
    One bulk CoinPaprika call, ranks INSIGHT_COINS by |24h % change| so we
    don't hit the API once per coin every hour.
    """
    try:
...
def build_vip_insight_message(trending_coins):
    lines = [f"👑 <b>MaveConnect VIP Market Insight</b> — {datetime.now().strftime('%H:%M UTC')}\n"]

    any_data = False
    for symbol in trending_coins:
        scan = scan_coin(symbol)
        if not scan:
            continue
        ...
        lines.append(
            f"<b>{scan['symbol']}</b> — {scan['coin']}\n"
            f"💰 ${scan['price']:,.4f}  {arrow} {scan['change24']:+.2f}%\n"
...
def build_free_insight_message(trending_coins):
    lines = [f"📊 <b>MaveConnect Market Update</b> — {datetime.now().strftime('%H:%M UTC')}\n"]

    any_data = False
    for symbol in trending_coins[:2]:
        data = get_coin_data(symbol)
        if not data:
            continue
        ...
        lines.append(f"{arrow} {data['symbol']}: ${data['price']:,.4f}  ({data['change24']:+.2f}% 24h)")

    if not any_data:
        return None

    lines.append("\n💎 Want RSI, trend & support/resistance? Tap below to join VIP.")
    lines.append(f"\n{MEMECOIN_TEASER}")
...
def insight_poster():
    ...
    while True:
        ...
        try:
            ...
            if trending:
                if VIP_GROUP_CHAT_ID:
                    vip_text = build_vip_insight_message(trending)
                    if vip_text:
                        try:
                            bot.send_message(VIP_GROUP_CHAT_ID, vip_text, parse_mode="HTML")
                        except Exception as e:
                            print(f"VIP Insight Send Error: {e}")
                else:
                    print("Insight poster: VIP_GROUP_CHAT_ID not configured yet.")

                if FREE_GROUP_CHAT_ID:
                    free_text = build_free_insight_message(trending)
                    if free_text:
                        try:
                            vip_upsell_markup = types.InlineKeyboardMarkup()
                            vip_upsell_markup.add(types.InlineKeyboardButton("💎 Join VIP", url=VIP_GROUP))
                            bot.send_message(
                                FREE_GROUP_CHAT_ID, free_text,
                                parse_mode="HTML", reply_markup=vip_upsell_markup
                            )
                        except Exception as e:
                            print(f"Free Insight Send Error: {e}")
                else:
                    print("Insight poster: FREE_GROUP_CHAT_ID not configured yet.")
            else:
                print("Insight poster: no trending coins found this cycle")

        except Exception as e:
            print(f"Insight Poster Error: {e}")
...
if __name__ == "__main__":
    ...
    Thread(target=weekly_leaderboard_prize, daemon=True).start()
