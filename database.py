...
ALTER TABLE plats
ADD COLUMN IF NOT EXISTS luck_points BIGINT DEFAULT 0;
""")
...
# ---- Collectibles ----
# Cosmetic-only items (badges/skins) with rarity tiers. Deliberately NOT
# framed as tradeable financial assets or NFTs — no secondary market, no
# resale, no claim of value appreciation. Just fun profile flair.
...
# ================= LUCK POINTS =================
# A SEPARATE, NON-CASHABLE currency used only by the chance-based games
# (Coin Flip, Dice Roll, Slot Machine, Lucky Number, Jackpot). Luck Points
# can be topped up from real Plats (one-way), but never convert back to
# Plats or real money — this keeps those games as pure entertainment,
# distinct from real-money gambling.

def get_luck_points(user_id):
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute(
        "SELECT luck_points FROM plats WHERE user_id=%s",
        (str(user_id),)
    )
...
def add_luck_points(user_id, amount):
    user_id = str(user_id)

    conn = get_connection()
    cursor = conn.cursor()

    current = get_luck_points(user_id)

    cursor.execute("""
        INSERT INTO plats(user_id, luck_points)
        VALUES(%s, %s)
        ON CONFLICT (user_id)
        DO UPDATE SET luck_points=%s
    """, (
        user_id,
        current + amount,
        current + amount
...
def remove_luck_points(user_id, amount):
    user_id = str(user_id)
...
def convert_plats_to_luck(user_id, plats_amount, luck_awarded):
    """
    One-way conversion: deducts real Plats, credits Luck Points.
    plats_amount and luck_awarded are passed in separately so the caller
    can apply a bonus rate (e.g. 1 Plat -> 1.5 Luck Points) without this
    function needing to know the current promo rate.
    """
    remove_plats(user_id, plats_amount)
    add_luck_points(user_id, luck_awarded)
...
def get_user_collectibles(user_id):
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT item_id, obtained_at FROM user_collectibles
        WHERE user_id=%s
...
def owns_collectible(user_id, item_id):
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT 1 FROM user_collectibles
...
def grant_collectible(user_id, item_id):
    """Adds an item to a user's collection. Returns False if they already
    own it (so it's safe to call blindly from a purchase or random drop)."""
    ...
    try:
        cursor.execute("""
            INSERT INTO user_collectibles(user_id, item_id)
            VALUES(%s, %s)
        ...
        conn.rollback()
        print("grant_collectible error:", e)
        return False
...
def get_active_vip_members():
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT user_id, vip_plan, vip_expiry
        FROM plats
        WHERE vip=TRUE
...
def get_all_user_ids():
    ...
    cursor.execute("SELECT user_id FROM plats")
